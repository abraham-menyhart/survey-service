...
public class MemberController {
    ...
    public ResponseEntity<List<Survey>> getCompletedSurveys(@PathVariable Long memberId) {
        List<Survey> surveys = completedSurveysService.fetchCompletedSurveysByMemberId(memberId);
        return ResponseEntity.ok(surveys);
    }

    @GetMapping("/{memberId}/points")
    public ResponseEntity<List<MemberPointsDto>> getMemberPoints(@PathVariable Long memberId) {
